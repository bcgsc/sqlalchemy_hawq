...
| hawq_partition_by | RangePartition or ListPartition | `ListPartition('chrom', {'chr1': '1', 'chr2':'2', 'chr3':'3'}, [RangeSubpartition('year', 2002, 2012, 1), RangeSubpartition('month', 1, 13, 1),])` | Expects RangePartition(column_name=str, start=int, end=int, every=int, subpartitions=[]) or \nListPartition(column_name=str, columns=dict{value_to_partition_on:name_of_partition}, subpartitions=[]), where subpartitions is an array of RangeSubpartition and ListSubpartition. \nSubpartitions expect the same params as Partitions but without a nested subpartition array. \nPartition level is determined by the order of the subpartitions in the subpartition array. \nDoes not currently support range partitioning on dates |
